...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val List(kafkaZkUrl, topic, kafkaConsumerThreadCount, kafkaStreamCount, outputPath) =
      List("10.62.14.27:2181,10.62.14.10:2181,10.62.14.28:2181", "applog", "10", "1", "hdfs://10.62.14.46:9000/tmp/applog")

    val partitionKeys = List("year", "month", "stack", "service")
    val sparkConf = new SparkConf().setMaster("local[8]").setAppName("log2hdfs")
    ...
    logStream.foreachRDD(rdd => {
      rdd.foreachPartition(partition => {
        ...
        partition.foreach(json => {
          ...
          val filePath = new StringBuilder().append(rootDir).append("/tmp/applog/")
